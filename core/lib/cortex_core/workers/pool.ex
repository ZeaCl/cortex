defmodule CortexCore.Workers.Pool do
  ...
  - Manejar failover cuando un worker falla (sistema de 3-Strikes)
  ...
  DEPRECATED: Health checks fueron removidos en v1.2.0.
  Los workers ahora usan el sistema de 3-Strikes para failover automático.
  Esta función se mantiene por backward compatibility y retorna %{}.
  """
  def health_status(pool \\ __MODULE__) do
    # Stub por backward compatibility - siempre retorna mapa vacío
    # El sistema de 3-Strikes maneja los fallos automáticamente sin health checks
    %{}
  end
  ...
  DEPRECATED: Health checks fueron removidos en v1.2.0.
  Esta función se mantiene por backward compatibility.
  """
  def check_health(pool \\ __MODULE__) do
    # Stub por backward compatibility - no hace nada
    :ok
  end
  ...
  @impl true
  def handle_cast({:mark_degraded, worker_name, :rate_limited}, state) do
    # Rate limiting - mark immediately as unavailable by setting high failure count
    Logger.warning("Worker #{worker_name} marcado como :rate_limited inmediatamente")

    {:noreply,
     %{state | failure_counts: Map.put(state.failure_counts, worker_name, @failure_threshold)}}
  end

  @impl true
  def handle_cast({:mark_degraded, worker_name, _status}, state) do
    # Generic error - apply 3-strikes logic
    current_count = Map.get(state.failure_counts, worker_name, 0)
    new_count = current_count + 1

    if new_count >= @failure_threshold do
      # Threshold reached - mark as unavailable
      Logger.error(
        "Worker #{worker_name} alcanzó #{new_count} fallos consecutivos, marcado como :unavailable"
      )
      ...
      Logger.warning("Worker #{worker_name} recibió strike #{new_count}/#{@failure_threshold}")
      {:noreply, %{state | failure_counts: Map.put(state.failure_counts, worker_name, new_count)}}
    end
  end

  @impl true
  def handle_cast({:report_success, worker_name}, state) do
    # Reset failure count on success
    Logger.debug("Worker #{worker_name} reportó éxito, reseteando contador de fallos")
    {:noreply, %{state | failure_counts: Map.put(state.failure_counts, worker_name, 0)}}
  end
  ...
  def handle_info(:periodic_health_check, state) do
    # Stub por backward compatibility - health checks fueron removidos
    {:noreply, state}
  end
  ...
  defp get_available_workers(state, service_type) do
    ...
    available = Enum.filter(workers_by_type, &worker_available?(&1, state.failure_counts))
  ...
  defp worker_available?(worker, failure_counts) do
    # Un worker está disponible si su contador de fallos es menor al threshold
    failure_count = Map.get(failure_counts, worker.name, 0)
  ...
  defp execute_with_failover([worker | rest], messages, opts, error_details) do
    ...
    case worker.__struct__.stream_completion(worker, messages, opts) do
      {:ok, stream} ->
        handle_stream_result(worker, stream, rest, messages, opts, error_details)
  ...
  defp handle_stream_result(worker, stream, rest, messages, opts, error_details) do
    case validate_stream(stream) do
      :ok ->
        handle_successful_stream(worker, stream, error_details, opts)

      error ->
        handle_failed_stream(worker, stream, error, rest, messages, opts, error_details)
    end
  end

  defp handle_successful_stream(worker, stream, error_details, opts) do
    Logger.info("Worker #{worker.name} respondió exitosamente")
    # Report success to reset failure counts
    GenServer.cast(__MODULE__, {:report_success, worker.name})
  ...
  defp handle_failed_stream(worker, _stream, error, rest, messages, opts, error_details) do
    error_msg = format_validation_error(error)

    Logger.warning("Worker #{worker.name} falló: #{error_msg}, intentando con siguiente worker")
  ...
  defp get_workers_by_provider(state, provider) do
    ...
    |> Enum.filter(fn worker ->
      ...
      failure_count = Map.get(state.failure_counts, worker_name, 0)

      # Worker disponible si failure_count < threshold
  ...
  defp get_workers_by_service_type(state, service_type) do
    ...
    filtered_workers =
      ...
      |> Enum.filter(fn worker ->
        ...
        # Worker disponible si failure_count < threshold
        is_available = failure_count < @failure_threshold
